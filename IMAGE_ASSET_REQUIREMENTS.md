...
| `public/images/team/tomislav-siketic.jpg` (prefer additional WebP/AVIF versions) | About/principal section | 1200 × 1500 px minimum | 4:5 portrait; head and shoulders with mobile-safe central crop | `Mag. Tomislav Siketic, Steuerberater und Geschäftsführer von MINO Consulting KG` | `Mag. Tomislav Siketic, porezni savjetnik i direktor društva MINO Consulting KG` | Must be the real adviser and explicitly approved for publication | Not supplied; owner must provide source and publication permission |
...
The hero placeholder is loaded eagerly with intrinsic dimensions and high fetch priority because it is the likely LCP image. The below-the-fold adviser composition is lazy-loaded and has empty alternative text; it must not be interpreted as Tomislav or as an actual employee. Replace placeholders only after the real files, identities and rights are confirmed. Keep intrinsic dimensions and responsive `sizes`, then provide AVIF/WebP renditions and `srcset`.

Do not use a generic professional portrait beside Tomislav’s name. Until an approved real portrait exists, the current abstract/local placeholder remains decorative.
